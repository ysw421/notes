...
\begin{frame}{기억의 종류}
  ...
  \begin{figure}
    ...
    %   }
    %   \node[red] at (0,5.1) {\large{Consciousness?}};
    %   %\node[red, align=left] at (0,4.5) {\large {Kept in mind during\\`delay period'?}};
    %   \node[red, align=left] at (0,3.5) {\large {Involve previous\\episode?}};
    %   \node[red, align=left] at (0,2.5) {\large {Task\\performance:}};
...
\subsection{장기 기억}
\begin{frame}{장기 기억\textsuperscript{Long-Term Memory}\\\large{: 오랜 시간 동안 정보를 저장하는 시스템}}
  \noindent
    ...
    \subfloat[Regions of the brain associated with episodic memory]
      {
        \begin{tikzpicture}
          \node[anchor=south west,inner sep=0] (image) at (0,0) {
            \includegraphics[height=2cm]{image/brain_regions}%
          }
          \draw<2->[red,very thick] (4.55,0.35) circle (0.29cm);
          \draw<3->[red,very thick] (0.9,1.3) circle (0.29cm);
          \draw<4->[red,very thick] (2.2,1.1) circle (0.29cm);
...
\subsection{작업 기억}
\begin{frame}{작업 기억\textsuperscript{Working Memory}\\\large : 짧은 시간 동안 정보를 유지(maintenance)하고, 그 정보를 조작/이용해 현재 과제를 수행하는 인지 시스템
}
...
\subsection{암묵 기억}
\begin{frame}{암묵 기억\textsuperscript{Implicit Memory}\\\large : 과거의 경험을 \textbf{의식적인 자각 없이} 현재의 과제 수행에 사용하는 기억}
  \textbf{특징}
  \vspace{-0.5em}
  \begin{itemize}
...
\begin{frame}{대표적인 실험 패러다임: Repetition Priming\\\large : 참가자에게 의식적으로 무언가를 기억해내라고 직접적으로 요구하지 않고, 간접적인 과제를 통해 기억의 효과를 측정하는 것}
  \large
  \textbf{기본 단계}
  \vspace{-0.5em}
  \begin{enumerate}
